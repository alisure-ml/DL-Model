...
def create_readable_names_for_imagenet_labels(synsets_filename=None, metadata_filename=None):
    ...
    filename = urllib.request.urlretrieve(synset_url)[0] if synsets_filename is None else synsets_filename
    synset_list = [s.strip() for s in open(filename).readlines()]
    num_synsets_in_ilsvrc = len(synset_list)
    assert num_synsets_in_ilsvrc == 1000

    filename = urllib.request.urlretrieve(synset_to_human_url)[0] if metadata_filename is None else metadata_filename
